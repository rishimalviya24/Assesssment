    ...
    <div className="border-b border-[#222]">
      <button onClick={onToggle} className="w-full bg-transparent border-0 cursor-pointer flex justify-between items-center py-6 sm:py-7 text-left gap-4">
        <span
          style={{
            ...
            fontWeight: 600,
            lineHeight: 1,
            color: isOpen ? '#e85c00' : '#fff',
            transition: 'color 0.3s'
          }}>
          {item.title}{item.sub && <span style={{fontWeight:400, marginLeft:8, fontSize:'clamp(12px,1.4vw,18px)'}}> ({item.sub})</span>}
        </span>
        <span className="text-[22px] sm:text-[24px] text-white flex-shrink-0 transition-transform duration-300"
          style={{ transform: isOpen ? 'rotate(45deg)' : 'none', fontFamily:'Switzer,sans-serif', fontWeight:300 }}>+</span>
      </button>
      <div style={{ overflow:'hidden', height:`${height}px`, transition:'height .4s cubic-bezier(.4,0,.2,1)' }}>
        <div ref={ref} className="pb-7 sm:pb-8 pl-0 sm:pl-[72px]">
          <p className="text-[14px] leading-[1.82] text-white/70 max-w-[520px] m-0" style={{fontFamily:'Switzer, sans-serif', fontWeight:400}}>
            {item.body}
          </p>
        </div>
      </div>
